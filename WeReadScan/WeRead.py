...
from .script import img2pdf, dir_check, os_start_file, clear_temp, addBookmark2pdf, resolve_confict_png_name
...
class WeRead:
    ...
    def refresh_toc_per_chapter(self, chapter_name, toc_chapters, toc, start_page, start_page_height):
        try:
            chapter_idx_tmp = toc_chapters.index(chapter_name)
            chapter_idx = len(list(zip(*toc))[0]) - (len(toc_chapters) - chapter_idx_tmp)
            del toc_chapters[0: chapter_idx_tmp + 1]
    ...
    def scan2pdf(self, book_url, save_at='.', binary_threshold=200, keep_color=False, show_output=True, font_size_index=1):
        ...
        toc_chapters = list(list(zip(*toc))[0])
        ...
        while True:
            ...
            # context_scan2png
            escaped_chapter_name = chapter.replace("/","_").replace("\\","_")
            png_name = f'wrs-temp/{book_name}/context/{escaped_chapter_name}'
            png_name = resolve_confict_png_name(png_name)
            ...
            # store toc
            toc = self.refresh_toc_per_chapter(chapter, toc_chapters, toc, len(png_name_list), list(zip(*split_pngs))[1][0])
        ...
        img2pdf(f'wrs-temp/{book_name}/{book_name}', png_name_list, binary_threshold, keep_color)
        if self.debug_mode:
            print(f"Book ToC: {toc}")
        addBookmark2pdf(f'wrs-temp/{book_name}/{book_name}', f'{save_at}/{book_name}', toc)
